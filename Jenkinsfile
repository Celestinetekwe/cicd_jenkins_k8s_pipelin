        ...
        stage('Deploy to EKS') {
            steps {
                script {
                    ...
                    sh "aws eks get-token --cluster-name ${EKS_CLUSTER_NAME} --region ${AWS_DEFAULT_REGION} | jq -r '.status.token' | pbcopy"
                    sh "kubectl auth can-i '*' '*' --all-namespaces --kubeconfig /tmp/kubeconfig"
